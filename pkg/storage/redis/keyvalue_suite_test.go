...
func (suite *KeyValueServiceTestSuite) TestSetEx() {
	...
	for _, tc := range testCases {
		suite.T().Run(tc.name, func(t *testing.T) {
			...
			if _, err := suite.KeyValueService.Get(tc.args.key); err != nil && err == cockpit.ErrKeyNotFound {
				t.Fatalf("KeyValueService.Get(notExpKey) err = %v; want nil", err)
			}

			time.Sleep(tc.args.exp + (100 * time.Millisecond))
			if _, err := suite.KeyValueService.Get(tc.args.key); err != cockpit.ErrKeyNotFound {
				t.Fatalf("KeyValueService.Get(expKey) err = nil; want %v", cockpit.ErrKeyNotFound.Error())
			}
		})
	}
}
func (suite *KeyValueServiceTestSuite) TestDelete() {
	...
	for _, tc := range testCases {
		suite.T().Run(tc.name, func(t *testing.T) {
			...
			if _, err := suite.KeyValueService.Get(tc.args.key); err != cockpit.ErrKeyNotFound {
				t.Fatalf("KeyValueService.Get(deletedKey) err = nil; want %v", cockpit.ErrKeyNotFound.Error())
			}
		})
	}
}
